...
import org.glowa.danube.components.actor.interfaces.ActorControllerToRekurTouristModel;
import org.glowa.danube.components.actor.interfaces.RekurTouristModelToActorController;
import org.glowa.danube.components.actor.touristmodel.deepactors.DATA_Destination;
import org.glowa.danube.components.actor.touristmodel.deepactors.DA_SourceArea;
import org.glowa.danube.components.actor.touristmodel.deepactors.DA_Tourist;
import org.glowa.danube.components.actor.touristmodel.gui.GUI;
...
public class TouristModel extends AbstractActorModel<TouristProxel> implements RekurTouristModelToActorController
{
   ...
   * Configurationfile can be found unter metadata/components/touristmodel.cfg
   */
  public String database = "jdbc:mysql://localhost/rekur?user=root&password=bla";
  /**
  ...
  public ActorControllerToRekurTouristModel controller; 
  public boolean destinationInit = true;
  public int weeksToForecast =52;
  public int categories = 7;
  public int startYear = 2011;
  public int startMonth = 1;
  public int startDay = 1;
  public GregorianCalendar currentDate;
  
  public int preSimulationTime = 0;
  ...
  public TouristModel(){
	...
	protected void init() {
		...
		try {
			controller = (ActorControllerToRekurTouristModel) getImport("org.glowa.danube.components.actor.interfaces.ActorControllerToRekurTouristModel");
		} catch (Exception ex) {
			this.logger().warn(ex);
		}
	}
	...
	public void setDestinationChanged(DA_Tourist tourist){
		for(int[] row:tourist.holidayDestination){
			...
			if(destinations.get(row[0]).CurrentNumberOfTourists.containsKey(tourist.origin)){
				...
				destinations.get(row[0]).CurrentNumberOfTourists.put(tourist.origin, new IntegerArray2D(weeksToForecast, categories));
				destinations.get(row[0]).CurrentNumberOfTourists.get(tourist.origin).array[row[1]][row[2]]++;
			}
		}
	}
