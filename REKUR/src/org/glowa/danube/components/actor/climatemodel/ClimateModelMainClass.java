...

{
	...
	/**
	 * Saves the IntegerDataTable with the tci of the current month.
	 */
	private IntegerDataTable tci;
	...
	/**
	 * holds the name of the relation containing the cidtemp attributes. Given in the config-file.
	 * Configuration file can be found under metadata/components/touristmodel.cfg
	 */
	private String cidtemp;
	/**
	 * holds the name of the relation containing the cidrating attributes. Given in the config-file.
	 * Configuration file can be found under metadata/components/touristmodel.cfg
	 */
	private String cidrating;
	/**
	 * holds the name of the relation containing the sourcearea attributes. Given in the config-file.
	 * Configuration file can be found under metadata/components/touristmodel.cfg
	 */
	private String preciprating;
	/**
	 * holds the name of the relation containing the preciprating attributes. Given in the config-file.
	 * Configuration file can be found under metadata/components/touristmodel.cfg
	 */
	private String windrating;
	/**
	 * holds the name of the relation containing the windrating815 attributes. Given in the config-file.
	 * Configuration file can be found under metadata/components/touristmodel.cfg
	 */
	private String windrating815;
	/**
	 * holds the name of the relation containing the sunrating attributes. Given in the config-file.
	 * Configuration file can be found under metadata/components/touristmodel.cfg
	 */
	private String sunrating;
	
	/**
	 * Saves the ratings per category;
	 */
	public static float[][] precipratings = new float[11][3];
	/**
	 * Saves the ratings per category;
	 */
	public static float[][] windratings = new float[9][5];
	/**
	 * Saves the ratings per category;
	 */
	public static float[][] windratings815 = new float[8][2];
	/**
	 * Saves the ratings per category;
	 */
	public static float[][] sunratings = new float[11][3];
	/**
	 * Saves the ratings per category;
	 */
	public static float[][] cidtemps = new float[11][16];
	/**
	 * Saves the ratings per category;
	 */
	...
	protected void init() {
		...
		tci = new IntegerDataTable(areaMetaData());
	    ...
	    cidtemp = this.componentConfig().getComponentProperties().getProperty("cidtemp");
		cidrating = this.componentConfig().getComponentProperties().getProperty("cidrating");
		preciprating = this.componentConfig().getComponentProperties().getProperty("preciprating");
		windrating = this.componentConfig().getComponentProperties().getProperty("windrating");
		windrating815 = this.componentConfig().getComponentProperties().getProperty("windrating815");
		sunrating = this.componentConfig().getComponentProperties().getProperty("sunrating");
		
		readTCIRatings();
		
//		checkTables(cidtemps);
//		checkTables(cidratings);
//		checkTables(windratings);
//		checkTables(sunratings);
//		checkTables(precipratings);
//		checkTables(windratings815);
...
//		netCDFReader.initFile(climateFolderPath+meanTempFileName);
	...
	private void readTCIRatings(){
		cidtemps=readValuesFromDatabase(cidtemp, cidtemps);
		cidratings=readValuesFromDatabase(cidrating, cidratings);
		windratings=readValuesFromDatabase(windrating, windratings);
		sunratings=readValuesFromDatabase(sunrating, sunratings);
		precipratings=readValuesFromDatabase(preciprating, precipratings);
		windratings815=readValuesFromDatabase(windrating815, windratings815);
	}
	
	private float[][] readValuesFromDatabase(String relationName, float[][] table){
		try{
			...
			while(sa.next()){
				for(int x = 0;x<table[i].length;x++){
					table[i][x] = Float.parseFloat((sa.getString(x+1).replaceAll(",", ".")));
				}
				i++;
			}
			return table; 
	...
	private void checkTables(float[][]table){
		for(float[] y : table){
			for(float x: y){
				System.out.print(x+" ");
			}
...
//		System.out.println("ClimateCompute"+actTime.getDay()+actTime.getMonth()+actTime.getYear());
		//Data-Objekt zur Uebergabe an Proxel mit netCDFReader-Objekt verknŸpfen, da dort die KlimaArrays vorliegen.
